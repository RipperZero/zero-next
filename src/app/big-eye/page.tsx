    ...
    wakeUpTimeoutId.current = setInterval(() => {
      setEyeBallChartParams(
        produce((draft) => {
          ...
          // 旋转
          draft.leftRotSize >= 360
            ? (draft.leftRotSize = 0)
            : (draft.leftRotSize = plus(draft.leftRotSize, 0.5));
        }),
      );
    }, 10);
  });
      ...
      const rotTimeId = setInterval(() => {
        setEyeBallChartParams(
          produce((draft) => {
            ...
            draft.leftRotSize >= 360
              ? (draft.leftRotSize = 0)
              : (draft.leftRotSize = plus(draft.leftRotSize, 0.1));
    ...
    >
      <div
        ...
        className={clsx(
          ...
          isAngry &&
            "animate-[eyeLoockAround_2.5s] border-[#ffbbff] shadow-[#ff3c56]",
        )}
      ...
      <div style={{ filter: "url('#filter')" }} className="h-full w-full">
        <div
          className={clsx(
            ...
            isSleep && "animate-[eyeSleeping_6s_infinite]",
            isAngry &&
              "animate-[eyeLoockAround_2.5s] border-[#ffbbff] opacity-100 shadow-[#ff3c56]",
          )}
        ></div>
      </div>
